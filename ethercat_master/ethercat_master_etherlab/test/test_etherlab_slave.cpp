...
/*TEST_F(EtherlabSlaveTest, EcReadTPDOToStateInterface)
{
  ...
  test_slave_ptr->state_interface_ptr_ = &state_interface;
  slave_parameters["state_interface/effort"] = "1";
  test_slave_ptr->parameters_ = slave_parameters;

  ASSERT_TRUE(test_slave_ptr->setup_from_config(YAML::Load(test_slave_config)));
  test_slave_ptr->setup_interface_mapping();

  ASSERT_EQ(test_slave_ptr->get_pdo_channels_info()[8]->state_interface_index(), 1);
  uint8_t domain_address[2];
  write_s16(domain_address, 42);
  test_slave_ptr->process_data(8, domain_address);
  ASSERT_EQ(test_slave_ptr->state_interface_ptr_->at(1), 5 * 42 + 15);
}
