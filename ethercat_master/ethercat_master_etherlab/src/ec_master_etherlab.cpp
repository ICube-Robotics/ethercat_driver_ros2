...
namespace ethercat_master
{
  ...
  bool EtherlabMaster::add_slave(std::shared_ptr<ethercat_interface::EcSlaveBase> slave)
  {
    ...
    {
      std::string error_message = "Alias and position not set for slave (vendor id=" + std::to_string(slave->get_vendor_id()) + ",product_code=" + std::to_string(slave->get_product_id()) + ").";
  ...
  void EtherlabMaster::registerTransferInDomain(const std::vector<ethercat_interface::EcTransferNet> &transfer_nets)
  {
      ...
      for (auto & transfer : net.transfers) {
        ...
        for (const auto & key_val : domain_info_) {
          ...
          for (auto & domain_reg : domain.domain_regs) {
            ...
            {
              transfer_info.input_domain = (void*)&domain;
            ...
            {
              transfer_info.output_domain = (void*)&domain;
              // 3. Compute the pointer arithmetic and store the result in the EcTransferInfo object
