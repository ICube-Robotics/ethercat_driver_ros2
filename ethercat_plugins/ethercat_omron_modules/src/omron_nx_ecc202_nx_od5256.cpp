    ...
    std::vector<double> * command_interface)
  {
    ...
    for (auto index = 0ul; index < 16; index++) {
      if (paramters_.find("do." + std::to_string(index)) != paramters_.end()) {
        if (paramters_.find(
            "command_interface/" + paramters_["do." +
            std::to_string(index)]) != paramters_.end())
        {
          cii_do_[index] = std::stoi(
            paramters_["command_interface/" + paramters_["do." + std::to_string(index)]]);
        }
        if (paramters_.find(
            "state_interface/" + paramters_["do." +
            std::to_string(index)]) != paramters_.end())
        {
...
private:
  std::vector<int> cii_do_{std::vector<int>(16, -1)};
  std::vector<int> sii_do_{std::vector<int>(16, -1)};
