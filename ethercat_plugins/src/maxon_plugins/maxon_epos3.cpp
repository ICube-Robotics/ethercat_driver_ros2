...
{
public:
    Maxon_EPOS3() : EcSlave(0x000000fb, 0x64400000) {}
    virtual ~Maxon_EPOS3() {}
        ...
        switch(index){
            ...
            case 2:
                if(isTargetVelocityRequired && (
                  mode_of_operation_display_ == MODE_CYCLIC_SYNC_VELEOCITY ||
        ...
        if (index==18){ //if last entry  in domain
            if (status_word_ != last_status_word_){
                ...
                if (state_ != last_state_){
                    std::cout << "STATE: " << device_state_str_[state_] <<" with status word :" << status_word_ << std::endl;
                }
            }
