    ...
    std::vector<double> * command_interface)
  {
    ...
    for (auto index = 0ul; index < 4; index++) {
      if (paramters_.find("do." + std::to_string(index + 1)) != paramters_.end()) {
        if (paramters_.find("command_interface/" + paramters_["do." + std::to_string(index + 1)]) !=
          paramters_.end())
        {
          cii_do_[index] = std::stoi(
            paramters_["command_interface/" + paramters_["do." + std::to_string(index + 1)]]);
        }
        if (paramters_.find("state_interface/" + paramters_["do." + std::to_string(index + 1)]) !=
          paramters_.end())
        {
...
private:
  ...
  ec_sync_info_t syncs_[2] = {{0, EC_DIR_OUTPUT, 4, pdos_ + 0, EC_WD_ENABLE}, {0xff}};
