...
public:
  ...
  virtual void processData(size_t index, uint8_t * domain_address)
  {
    ...
    if (isRequested) {
      double data = command_interface_ptr_->at(
        std::stoi(
      ...
      state_interface_ptr_->at(
        std::stoi(
          paramters_["state_interface/" + paramters_["ao." + std::to_string(index + 1)]])) = data;
      int16_t dac_data = (int16_t)(data * static_cast<double>(65534 / 20));
...
private:
  ...
  ec_sync_info_t syncs_[2] = {{2, EC_DIR_OUTPUT, 4, pdos_ + 0, EC_WD_ENABLE}, {0xff}};
  DomainMap domains_ = {{0, {0, 1, 2, 3}}};
};
//---------------------------------------------------------------------------------------------------------
...
private:
  ...
  ec_sync_info_t syncs_[2] = {{2, EC_DIR_OUTPUT, 2, pdos_ + 0, EC_WD_ENABLE}, {0xff}};
  DomainMap domains_ = {{0, {0, 1}}};
};
