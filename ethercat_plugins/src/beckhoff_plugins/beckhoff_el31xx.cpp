...
public:
  ...
  {
    auto isRequested =
      paramters_.find("ai." + std::to_string(static_cast<int>(index + 1))) != paramters_.end();
    if (isRequested) {
      double data = static_cast<double>(EC_READ_S16(domain_address) / 32767 * 10);
      state_interface_ptr_->at(
        std::stoi(
          paramters_["state_interface/" +
          paramters_["ai." + std::to_string(static_cast<int>(index + 1))]])) = data;
    }
  }
...
private:
  ...
  ec_sync_info_t syncs_[2] = {{3, EC_DIR_INPUT, 4, pdos_ + 0, EC_WD_ENABLE}, {0xff}};
  DomainMap domains_ = {{0, {10, 21, 32, 43}}};
};
