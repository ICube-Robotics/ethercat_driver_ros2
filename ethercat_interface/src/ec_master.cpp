...
void EcMaster::addSlave(uint16_t alias, uint16_t position, EcSlave* slave)
{
    ...
    if(slave->use_dc_sync()){
        ...
        ecrt_slave_config_dc(slave_info.config,
                                0x0300,
                                interval_,
...
void EcMaster::registerPDOInDomain(uint16_t alias, uint16_t position,
...
void EcMaster::update(unsigned int domain)
{
...
void EcMaster::readData(unsigned int domain){
    // receive process data
    ecrt_master_receive(master_);
...
void EcMaster::writeData(unsigned int domain){
    DomainInfo* domain_info = domain_info_[domain];
    // read and write process data
    for (DomainInfo::Entry& entry : domain_info->entries){
        for (int i=0; i<entry.num_pdos; ++i){
...
unsigned long long EcMaster::elapsedCycles()
{
...
void EcMaster::checkDomainState(unsigned int domain)
{
    DomainInfo* domain_info = domain_info_[domain];
